...
def update_summary(inv_id: int, summary: str, refined_query: str = None, model: str = None, preset_label: str = None) -> None:
    """Update the summary (and optionally other metadata) for an investigation."""
    with _connect() as conn:
        if refined_query is not None:
            conn.execute("UPDATE investigations SET summary=?, refined_query=?, model=?, preset=? WHERE id=?", (summary, refined_query, model or '', preset_label or '', inv_id))
        else:
            conn.execute("UPDATE investigations SET summary=? WHERE id=?", (summary, inv_id))
